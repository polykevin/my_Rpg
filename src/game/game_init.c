...
static void init_depth_sprite(game_t *g)
{
    sprite_init(&g->gate, "resource/map/gate.png",
        (sfIntRect){0, 0, 80, 64});
    sfSprite_setScale(g->gate.sprite, (sfVector2f){4, 4});
    sfSprite_setPosition(g->gate.sprite,
        (sfVector2f){2176, 1136});
    sprite_init(&g->left_light, "resource/map/left_light.png",
        (sfIntRect){0, 0, 24, 43});
    sfSprite_setScale(g->left_light.sprite, (sfVector2f){4, 4});
    sfSprite_setPosition(g->left_light.sprite,
        (sfVector2f){2960, 1970});
    sprite_init(&g->right_light, "resource/map/right_light.png",
        (sfIntRect){0, 0, 25, 38});
    sfSprite_setScale(g->right_light.sprite, (sfVector2f){4, 4});
    sfSprite_setPosition(g->right_light.sprite,
        (sfVector2f){1612, 1988});
}
...
void game_init(game_t *g)
{
    ...
    g->map_collision =
        sfImage_createFromFile("resource/map/collision_map.png");
}
